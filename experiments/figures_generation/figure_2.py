...
for i, m in enumerate(pd.unique(data["dim_measurement"])):
    if m >= 40:
        ...
        scores_q1 = []
        scores_q3 = []
        for n_matrice in pd.unique(data["n_matrices"])[:5]:
            current_data = data[
...
axs[0].legend(title="Dim. m", fontsize=6, title_fontsize=6, loc="lower right")
axs[0].set_ylim([0.6, 1])
axs[0].set_yticks([0.6, 0.7, 0.8, 0.9, 1.0])
axs[0].set_xticks([1, 2, 3, 4, 5])
axs[0].set_xlabel("Number of matrices")
axs[0].set_ylabel("Rec. score\nCompr. sensing")
axs[0].grid()
...
axs[1].plot(s_values, psnrs.mean(axis=1), label="reconstruction")
axs[1].fill_between(
    s_values,
...
axs[1].plot(s_values, psnrs_corrupted.mean(axis=1), label="observation")
axs[1].fill_between(
    s_values,
...
axs[2].plot(s_values, scores_weights.mean(axis=1))
axs[2].fill_between(
        s_values,
...
axs[2].set_ylabel("Weighted score\nInpainting")
axs[2].grid()
