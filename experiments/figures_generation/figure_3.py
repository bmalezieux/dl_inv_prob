...
for image in images:
    ...
    for i, sigma in enumerate(pd.unique(data_256["sigma"])):
        for name in pd.unique(data_supervised_256["name"]):
            ...
            axs[0, i].plot(
                1 - np.array(list(pd.unique(data_supervised_256["rho"]))),
                psnr,
                label=name,
            )
            axs[1, i].plot(
        ...
        for name in pd.unique(data_256["name"]):
            ...
            for rho in list(pd.unique(data_256["rho"])):
                ...
                psnr_ran.append(current_data.iloc[id]["psnr_rec_range"])
            axs[0, i].plot(
                1 - np.array(list(pd.unique(data_256["rho"]))),
                psnr,
                label=name,
            )
            axs[1, i].plot(
    ...
    fig.legend(
        ...
        handles=handles,
        loc="center right",
        bbox_to_anchor=(1.25, 0.52),
    )
    plt.tight_layout()
    plt.savefig(f"../figures/inpainting_single_image_full_2_{image}.pdf")
    # plt.show()
    plt.clf()
    # %%
    ...
    for name in pd.unique(data_256["name"]):
        ...
        for rho in list(pd.unique(data_256["rho"])):
            ...
            psnr_ran.append(current_data.iloc[id]["psnr_rec_range"])
        axs[0].plot(
            1 - np.array(list(pd.unique(data_256["rho"]))), psnr, label=name
        )
        axs[1].plot(
            1 - np.array(list(pd.unique(data_256["rho"]))),
            psnr_ker,
    ...
    plt.tight_layout()
    plt.savefig(f"../figures/inpainting_single_image_spaces_{image}.pdf")
