...
for image in images:
    ...
    data_supervised = pd.read_csv(
        "../results/deblurring_single_image_supervised.csv"
    )
    data_supervised = data_supervised[data_supervised["image"] == image]
    data_supervised_256 = data_supervised[data_supervised["size"] == 256]

    # %%
    fig, axs = plt.subplots(3, 4, figsize=(6, 4.5), sharey=True)
    ...
    for i, sigma in enumerate(pd.unique(data_256["sigma"])):
        for name in pd.unique(data_supervised_256["name"]):
            ...
            psnr_ker = []
            psnr_ran = []
            for sigma_blurr in list(
                pd.unique(data_supervised_256["sigma_blurr"])
            )[:-1]:
            ...
            axs[0, i].plot(
                list(pd.unique(data_supervised_256["sigma_blurr"]))[:-1],
                psnr,
                label=name,
            )
            axs[1, i].plot(
                list(pd.unique(data_supervised_256["sigma_blurr"]))[:-1],
                10 + np.array(psnr_ker),
                label=name,
            )
            axs[2, i].plot(
                list(pd.unique(data_supervised_256["sigma_blurr"]))[:-1],
        ...
        for name in pd.unique(data_256["name"]):
            ...
            for sigma_blurr in list(pd.unique(data_256["sigma_blurr"]))[:-1]:
                ...
                psnr_ran.append(current_data.iloc[id]["psnr_ran"])
            axs[0, i].plot(
                list(pd.unique(data_256["sigma_blurr"]))[:-1], psnr, label=name
            )
            axs[1, i].plot(
                list(pd.unique(data_256["sigma_blurr"]))[:-1],
                10 + np.array(psnr_ker),
                label=name,
            )
            axs[2, i].plot(
        ...
        axs[0, i].set_title(
            f"SNR {round(10 * np.log(0.205 ** 2 / (sigma ** 2)) / np.log(10), 0)}"
        )
        axs[0, i].grid()
        axs[1, i].grid()
        axs[2, i].grid()
    ...
    fig.legend(
        ...
        handles=handles,
        loc="center right",
        bbox_to_anchor=(1.23, 0.52),
    )
    plt.tight_layout()
    plt.savefig(f"../figures/deblurring_single_image_full_2_{image}.pdf")
    plt.clf()
    # %%
    ...
    for name in pd.unique(data_supervised_256["name"]):
        ...
        for sigma_blurr in list(pd.unique(data_supervised_256["sigma_blurr"])):
            ...
            psnr_ran.append(current_data.iloc[id]["psnr_ran"])
        axs[0].plot(
            np.array(list(pd.unique(data_supervised_256["sigma_blurr"]))),
            psnr,
            label=name,
        )
        axs[1].plot(
    ...
    for name in pd.unique(data_256["name"]):
        ...
        for sigma_blurr in list(pd.unique(data_256["sigma_blurr"])):
            ...
            psnr_ran.append(current_data.iloc[id]["psnr_ran"])
        axs[0].plot(
            np.array(list(pd.unique(data_256["sigma_blurr"]))),
            psnr,
            label=name,
        )
        axs[1].plot(
    ...
    fig.legend(
        ...
        handles=handles,
        loc="center right",
        bbox_to_anchor=(1.23, 0.56),
    )
    plt.tight_layout()
    plt.savefig(f"../figures/deblurring_single_image_spaces_{image}.pdf")
