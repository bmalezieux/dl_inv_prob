...
data = pd.read_csv("../results/deblurring_pnp.csv")
...
for i, sigma in enumerate(pd.unique(data["sigma_sample"])):
    ...
    psnr_supervised = []
    psnr_corr = []
    for sigma_blurr in list(pd.unique(data["sigma_blurr"]))[:-1]:
        current_psnr = data[
    ...
    axs[i].plot(list(pd.unique(data["sigma_blurr"]))[:-1], psnr_unsupervised, label="Unsupervised")
    axs[i].plot(list(pd.unique(data["sigma_blurr"]))[:-1], psnr_supervised, label="Supervised")
    axs[i].plot(list(pd.unique(data["sigma_blurr"]))[:-1], psnr_corr, label="Corrupted")
