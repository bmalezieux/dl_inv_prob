...
for i, sigma in enumerate(pd.unique(data_256["sigma"])):
    for name in pd.unique(data_supervised_256["name"]):
        ...
        axs[0, i].plot(list(pd.unique(data_supervised_256["rho"]))[:-1], psnr, label=name)
    for name in pd.unique(data_256["name"]):
        ...
        for rho in list(pd.unique(data_256["rho"]))[:-1]:
            ...
            psnr.append(current_psnr)
        axs[0, i].plot(list(pd.unique(data_256["rho"]))[:-1], psnr, label=name)
...
for i, sigma in enumerate(pd.unique(data_256["sigma"])):
    for name in pd.unique(data_supervised_256["name"]):
        ...
        axs[1, i].plot(list(pd.unique(data_supervised_256["rho"]))[:-1], is_div, label=name)
    for name in pd.unique(data_256["name"]):
        is_div = []
        for rho in list(pd.unique(data_256["rho"]))[:-1]:
            current_is = data_256[
            ...
            is_div.append(current_is)
        axs[1, i].plot(list(pd.unique(data_256["rho"]))[:-1], is_div, label=name)
