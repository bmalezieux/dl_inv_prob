...
RESULTS = os.path.join(EXPERIMENTS, "results/num_measurements.csv")
mem = Memory(location='./tmp_num_measurements/', verbose=0)
...
def run_test(params):
    ...
    for _ in range(N_EXP):
        ...
        A = rng.normal(size=(n_matrices, dim_measurement, dim_signal))
        A /= np.linalg.norm(A, axis=1, keepdims=True)

        Y, _ = generate_data(A @ D, N=n_data // n_matrices, s=sparsity, rng=rng)
